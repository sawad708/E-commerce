...
def add_to_cart(request,id):
    ...
    if not product.is_availiable or product.stock <=0 :
    ...
    if not created:
        if product is not None and cart_item.quantity >= product.stock:
...
def increment_cart_item(request,id):
    ...
    if cart_item.product is not None and cart_item.quantity >= cart_item.product.stock:
